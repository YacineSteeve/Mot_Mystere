...
bool ask_retry_win();              //Demande si le joueur veut recommencer la partie s'il a gagné.
...
void again(bool &ok, bool &retry); //Prend en argument les booléens "ok" et "retry" passés par référence.
                                   //Selon leurs valeurs, oriente le joueur selon s'il a gagné ou pas.

bool ask_replay();                 //Demande au joueur s'il veut retourner au menu (pour changer de mode de jeu).
                                   //Renvoie "vrai" si oui, et "faux" sinon.
...
vector<char> shuffle(string mot_mystere);   //Prend en argument le mot mystère.
...
void mot_alea(string &mot_mystere, bool &ok);   //Prend en argument la variable du mot mystère et le booléen "ok" passés par référence.
